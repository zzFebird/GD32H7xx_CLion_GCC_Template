...
void norflash_init(void)
{
    ...
    if (g_norflash_type == W25Q256)         /* SPI FLASH为W25Q256, 必须使能4字节地址模式 */
    {
        ...
        if ((temp & 0X01) == 0)             /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            ...
            /* SPI,发送使能4字节地址指令,地址为0,0个字节数据,无数据_8位地址_无地址_单线传输指令,无空周期 */
...
{  
    /* 发送退出QPI模式指令,地址为0,0个字节数据,无数据_8位地址_无地址_4线传输指令,无空周期 */
...
{
    /* SPI,发送写使能指令,地址为0,0个字节数据,无数据_8位地址_无地址_单线传输指令,无空周期 */
...
{
    /* SPI,发送写禁止指令,地址为0,0个字节数据,无数据_8位地址_无地址_单线传输指令,无空周期 */
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    /* SPI,发送读寄存器指令,地址为0,1个字节数据,单线传数据_8位地址_无地址_单线传输指令,无空周期 */
    ospi_send_command(command, 0, 1, (1 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
    ospi_receive(OSPI_INTERFACE, &byte);   /* 读取一个字节 */
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    /* SPI,发送写状态寄存器指令,地址为0,1个字节数据,单线传数据_8位地址_无地址_单线传输指令,无空周期 */
    ospi_send_command(command, 0, 1, (1 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
    ospi_transmit(OSPI_INTERFACE, &sr);   /* 写入一个字节 */
}
...
uint16_t norflash_read_id(void)
{
    ...
    /* SPI,发送读ID指令,地址为0,2个字节数据,单线传输数据_24位地址_单线传输地址_单线传输指令,无空周期 */
    ospi_send_command(FLASH_ManufactDeviceID, 0, 2, (1 << 8) | (2 << 6) | (1 << 3) | (1 << 0), 0);
    ospi_receive(OSPI_INTERFACE, temp);   /* 读取两个字节数据 */
...
{
    /* QPI,发送快速读数据指令,地址为addr,datalen个数据,4线传输数据_24/32位地址_4线传输地址_1线传输指令,6个空周期 */
    ospi_send_command(FLASH_FastReadQuad_IO, addr, datalen, (3 << 8) | (g_norflash_addrw << 6) | (3 << 3) | (1 << 0), 6);
    ospi_receive(OSPI_INTERFACE, pbuf);  /* 读取数据 */
}
...
void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint32_t datalen)
{
    ...
    /* QPI,发送写页指令,地址为addr,datalen个数据,4线传输数据_24/32位地址_1线传输地址_1线传输指令,无空周期 */
    ospi_send_command(FLASH_PageProgramQuad, addr, datalen, (3 << 8) | (g_norflash_addrw << 6) | (1 << 3) | (1 << 0), 0);
  
    ospi_transmit(OSPI_INTERFACE, pbuf);  /* 发送数据 */
...
void norflash_erase_chip(void)
{
    ...
    /* SPI,发送全片擦除指令,地址为0,0个字节数据,无数据_8位地址_无地址_1线传输指令,无空周期 */
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    /* SPI,发送扇区擦除指令,地址为0,0个字节数据,无数据_24/32位地址_1线传输地址_1线传输指令,无空周期 */
